...
namespace Servidor.DAL
{
    ...
    public class SpedFiscalDAL
    {
        ...
        public void GerarBloco0()
        {
            ...
            //consultaSql = "from ViewSpedNfeItemDTO item where Id in (Select det.IdProduto from NfeDetalheDTO as det inner join det.NfeCabecalho as cab where cab.DataHoraEmissao BETWEEN " + Biblioteca.QuotedStr(DataInicial) + " and " + Biblioteca.QuotedStr(DataFinal) + ")";
            IList<ViewSpedNfeItemDTO> ListaProduto = new NHibernateDAL<ViewSpedNfeItemDTO>(Session).Select(new ViewSpedNfeItemDTO());
        ...
        public void GerarBlocoC()
        {
            ...
            if (PerfilApresentacao == 0)
            {
                if (ListaNFeCabecalho != null)
                {
                    foreach (NfeCabecalhoDTO NFeCabecalho in ListaNFeCabecalho)
                    {
                        ...
                        if (NFeCabecalho.Fornecedor != null)
		    ...
		    // {Implementado pelo participante - SAT Fiscal}
        ...
        {
            /*
             * Implementado a critério do Participante
             * 
